...
"""
 В целях  решения данного задания написана функция подсчёта "My_getsizeof" размера переменных в памяти 
"""
...
def Max_count_1(my_list, show):
    ...
    My_getsizeof(my_list, show, count, count_list, i, n, ind_0, ind_n)
...
def Max_count_2(my_list, show):
    ...
    for i in my_list:
        ...
        if len(count_list) == len(my_list):
            ...
            My_getsizeof(my_list, show, count_list, count, i, n, const, my_index, ind, item)
...
def Max_count_3(my_list, show) -> dict:
    ...
    My_getsizeof(my_list, show, my_dict, count, i, n, num_count, num, cn)
...
def Max_count_4(my_list, show) -> str:
    ...
    My_getsizeof(my_list, show, my_dict, count, i, n, max_value)
...
def Max_count_5(my_list, show):
    ...
    My_getsizeof(my_list, show, num, frequency, i, spam, j)
...
print(f'\n {simb} Решение 1 {simb}')
print(Max_count_1(my_list, 0))
print(f'\n {simb} Решение 2 {simb}')
print(Max_count_2(my_list, 0))
print(f'\n {simb} Решение 3 {simb} ')
print(Max_count_3(my_list, 0))
print(f'\n {simb} Решение 4 {simb} ')
print(Max_count_4(my_list, 0))
print(f'\n {simb} Решение 5 {simb} ')
print(Max_count_5(my_list, 0))
...
"""
 Python версия 3.9(64-bit), разрядность ОС 64 

Рейтинг решений по размеру используемой памяти:
    - решение 5 - 544
    - решение 1 - 916
    - решение 2 - 972
    - решение 4 - 1_152
...
Рейтинг решений после асимптотиеского анализа от лучшего к худшему ( - (максимальное время)):
    - решение 3 - (1.09) 
    - решение 4 - (1.50)
    - решение 5 - (30.3)
    - решение 1 - (74.1)
    ...
    Анализируя асимптотику решений и раход памяти, решение 3 занимает меньше всего времени но больше всего памяти, 
        причина в использовании словарей которые содержат указатели на ключ и значение тем самым занимают больше памяти.
    При выборе решения можно вывести условие исходя из которых сформировать параметры отбора, так при работе с большим
    объёмом информации лучше использовать решение 3 так как время выполнения алгоритма имеет приоритет над памятью, 
    при работе с не большим объёмом информации можно использовать решение 5, где время выполнения алгоритма будет 
    не существенно увеличено но при этом будет меньше нагружать память в пользу более приоритетных опреаций.

-- расход памяти
 ============================== Решение 1 ==============================
 Используемые переменные: ['my_list', 'show', 'count', 'count_list', 'i', 'n', 'ind_0', 'ind_n'] 
 Кол-во переменных: 8 
 Расходовано памяти(байт): 916

 ============================== Решение 2 ==============================

 Используемые переменные: ['my_list', 'show', 'count_list', 'count', 'i', 'n', 'const', 'my_index', 'ind', 'item'] 
 Кол-во переменных: 10 
 Расходовано памяти(байт): 972

 ============================== Решение 3 ============================== 
 Используемые переменные: ['my_list', 'show', 'my_dict', 'count', 'i', 'n', 'num_count', 'num', 'cn'] 
 Кол-во переменных: 9 
 Расходовано памяти(байт): 1412

 ============================== Решение 4 ============================== 
 Используемые переменные: ['my_list', 'show', 'my_dict', 'count', 'i', 'n', 'max_value'] 
 Кол-во переменных: 7 
 Расходовано памяти(байт): 1152

 ============================== Решение 5 ============================== 
 Используемые переменные: ['my_list', 'show', 'num', 'frequency', 'i', 'spam', 'j'] 
 Кол-во переменных: 7 
 Расходовано памяти(байт): 544
